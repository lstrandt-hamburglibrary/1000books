...
// Pattern Builder Game - Clean rewrite
...
function launchPatternBuilderGame() {
    ...
    patternGameState = { level: 1, index: 0, score: 0 };

    const gameHTML = `
        <div id="gameModal" class="game-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; z-index: 3000;">
            <div class="game-container" style="background: white; padding: 2rem; border-radius: 20px; max-width: 700px; width: 90%; max-height: 90vh; overflow-y: auto;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h2>🔷 Pattern Builder</h2>
                    <button onclick="closeMiniGame()" style="background: #e74c3c; color: white; border: none; padding: 0.5rem 1rem; border-radius: 10px; cursor: pointer; font-size: 1rem;">✕ Close</button>
                </div>
                <div id="patternLevelSelect">
                    <p style="text-align: center; margin-bottom: 2rem; font-size: 1.1rem;">Complete the pattern by choosing the correct shape!</p>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                        <button onclick="selectPatternLevel(1)" style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); border: none; padding: 2rem; border-radius: 15px; cursor: pointer; font-size: 1.2rem; font-weight: bold;">
                            Level 1<br><span style="font-size: 0.9rem; font-weight: normal;">Simple Patterns</span>
                        </button>
                        <button onclick="selectPatternLevel(2)" style="background: linear-gradient(135deg, #fbc2eb 0%, #a6c1ee 100%); border: none; padding: 2rem; border-radius: 15px; cursor: pointer; font-size: 1.2rem; font-weight: bold;">
                            Level 2<br><span style="font-size: 0.9rem; font-weight: normal;">Trickier Patterns</span>
                        </button>
                        <button onclick="selectPatternLevel(3)" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border: none; padding: 2rem; border-radius: 15px; cursor: pointer; font-size: 1.2rem; font-weight: bold;">
                            Level 3<br><span style="font-size: 0.9rem; font-weight: normal;">Long Patterns</span>
                        </button>
                    </div>
                </div>
...
function selectPatternLevel(level) {
    patternGameState.level = level;
    patternGameState.index = 0;
...
function loadPatternPuzzle() {
    const levelKey = `level${patternGameState.level}`;
    const puzzles = PATTERN_LEVELS[levelKey];

    if (patternGameState.index >= puzzles.length) {
        showPatternComplete();
        return;
    }

    const puzzle = puzzles[patternGameState.index];
    const wrongOptions = PATTERN_SHAPES.filter(s => s !== puzzle.answer).sort(() => Math.random() - 0.5).slice(0, 3);
    ...
    gameArea.innerHTML = `
        <div style="text-align: center; margin-bottom: 1.5rem;">
            <h3>Pattern ${patternGameState.index + 1} of ${puzzles.length}</h3>
            <p>Score: ${patternGameState.score}</p>
        </div>
        <div style="background: #f8f9fa; padding: 2rem; border-radius: 15px; margin-bottom: 2rem; text-align: center;">
            <p style="font-weight: bold; margin-bottom: 1rem; color: #6c757d;">Complete the pattern:</p>
            <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap; font-size: 3rem;">
                ${puzzle.pattern.map(item => `<div style="width: 80px; height: 80px; display: flex; align-items: center; justify-content: center; background: white; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">${item}</div>`).join('')}
            </div>
        </div>
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 1rem;">
            ${allOptions.map(shape => `
                <button onclick="checkPatternAnswer('${shape}')" style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none; padding: 2rem; border-radius: 10px; cursor: pointer; font-size: 3rem;">
                    ${shape}
                </button>
            `).join('')}
        </div>
...
function checkPatternAnswer(selectedShape) {
    const levelKey = `level${patternGameState.level}`;
    const puzzle = PATTERN_LEVELS[levelKey][patternGameState.index];

    if (selectedShape === puzzle.answer) {
        patternGameState.score += 10;
        if (typeof showToast === 'function') showToast('✅ Correct!');
        patternGameState.index++;
        setTimeout(loadPatternPuzzle, 500);
    } else {
        if (typeof showToast === 'function') showToast('❌ Try again!');
    }
}
...
function showPatternComplete() {
    ...
    gameArea.innerHTML = `
        <div style="text-align: center; padding: 2rem;">
            <h2 style="color: #28a745; margin-bottom: 1rem;">🎉 Level Complete!</h2>
            <p style="font-size: 1.5rem; margin-bottom: 2rem;">Final Score: ${patternGameState.score}</p>
...
// Trace & Match Game - Clean rewrite
const TRACE_LETTERS = {
    level1: [
        ...
        <div id="gameModal" class="game-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; z-index: 3000;">
            <div class="game-container" style="background: white; padding: 2rem; border-radius: 20px; max-width: 700px; width: 90%; max-height: 90vh; overflow-y: auto;">
                ...
                <div id="traceLevelSelect">
                    <p style="text-align: center; margin-bottom: 2rem; font-size: 1.1rem;">Trace the letter, then match it to the right word!</p>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                        <button onclick="selectTraceLevel(1)" style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); border: none; padding: 2rem; border-radius: 15px; cursor: pointer; font-size: 1.2rem; font-weight: bold;">
                            Level 1<br><span style="font-size: 0.9rem; font-weight: normal;">Uppercase Letters</span>
                        </button>
                        <button onclick="selectTraceLevel(2)" style="background: linear-gradient(135deg, #fbc2eb 0%, #a6c1ee 100%); border: none; padding: 2rem; border-radius: 15px; cursor: pointer; font-size: 1.2rem; font-weight: bold;">
                            Level 2<br><span style="font-size: 0.9rem; font-weight: normal;">Lowercase Letters</span>
                        </button>
                        <button onclick="selectTraceLevel(3)" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border: none; padding: 2rem; border-radius: 15px; cursor: pointer; font-size: 1.2rem; font-weight: bold;">
                            Level 3<br><span style="font-size: 0.9rem; font-weight: normal;">Mixed Case</span>
                        </button>
                    </div>
                </div>
...
function selectTraceLevel(level) {
    traceGameState.level = level;
    traceGameState.index = 0;
    traceGameState.score = 0;
...
function loadTraceLetter() {
    const levelKey = `level${traceGameState.level}`;
    const letters = TRACE_LETTERS[levelKey];

    if (traceGameState.index >= letters.length) {
        showTraceComplete();
        return;
    }

    const letterData = letters[traceGameState.index];
    traceGameState.hasTraced = false;

    // Create wrong options from other letters at this level
    const allMatches = letters.flatMap(l => l.matches);
    const wrongMatches = allMatches.filter(m => !letterData.matches.includes(m)).sort(() => Math.random() - 0.5).slice(0, 2);
    const allOptions = [...letterData.matches, ...wrongMatches].sort(() => Math.random() - 0.5);

    const gameArea = document.getElementById('traceGameArea');
    gameArea.innerHTML = `
        <div style="text-align: center; margin-bottom: 1.5rem;">
            <h3>Letter ${traceGameState.index + 1} of ${letters.length}</h3>
            <p>Score: ${traceGameState.score}</p>
        </div>
        <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 15px; margin-bottom: 1.5rem; text-align: center;">
            <p style="font-weight: bold; margin-bottom: 0.5rem; color: #6c757d;">Practice writing this letter:</p>
            <div style="font-size: 8rem; font-weight: bold; color: #dee2e6; margin: 1rem 0; font-family: 'Comic Sans MS', cursive;">${letterData.letter}</div>
            <canvas id="traceCanvas" width="300" height="200" style="border: 3px dashed #007bff; border-radius: 10px; background: white; cursor: crosshair; touch-action: none; display: block; margin: 0 auto;"></canvas>
            <button onclick="clearTraceCanvas()" style="background: #6c757d; color: white; border: none; padding: 0.5rem 1rem; border-radius: 10px; cursor: pointer; margin-top: 0.5rem; font-size: 0.9rem;">🗑️ Clear</button>
        </div>
        <div style="margin-top: 1.5rem;">
            <p style="font-weight: bold; text-align: center; margin-bottom: 1rem;">Match "${letterData.letter}" to a word that starts with this letter:</p>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 1rem;">
                ${allOptions.map(match => `
                    <button onclick="checkTraceMatch('${match.replace(/'/g, "\\'")}', '${letterData.letter}')" style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none; padding: 1rem; border-radius: 10px; cursor: pointer; font-size: 0.9rem; font-weight: bold;">
                        ${match}
                    </button>
                `).join('')}
            </div>
    ...
    // Initialize canvas after a short delay to ensure it's in DOM
...
function checkTraceMatch(selectedMatch, correctLetter) {
    const levelKey = `level${traceGameState.level}`;
    const letterData = TRACE_LETTERS[levelKey][traceGameState.index];

    if (letterData.matches.includes(selectedMatch)) {
        traceGameState.score += 10;
        if (typeof showToast === 'function') showToast('✅ Correct match!');
        traceGameState.index++;
        setTimeout(loadTraceLetter, 500);
    } else {
        if (typeof showToast === 'function') showToast('❌ Try again!');
    }
}
